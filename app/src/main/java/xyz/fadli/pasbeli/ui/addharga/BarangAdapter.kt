...
class BarangAdapter(
        context: Context?,
        resource: Int,
        objects: List<BarangNamaDISTINCTGet>?
) : ArrayAdapter<BarangNamaDISTINCTGet>(context, resource, objects) {
    ...
    inner class AdapterHolder(private val textView: TextView) {
        ...
        fun bind(item: BarangNamaDISTINCTGet) {
            textView.text = item.nama_barang
        }
    }
}